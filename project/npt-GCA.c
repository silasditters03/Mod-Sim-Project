...
int main(int argc, char* argv[]){
    ...
    // New trackers for our hybrid statistics
    ...
    for (int step = 0; step < mc_steps; ++step) {
        ...
        // --- 1. CHOOSE SWEEP TYPE ---
        // Roll a random number between 0.0 and 1.0 to decide the move
        ...
        // --- 2. NPT VOLUME MOVE ---
        // Attempt one volume change per overall step, regardless of what sweep happened
        ...
        if (step % output_steps == 0) {
            // Calculate acceptances safely to avoid division by zero
            ...
            printf("%d \t Vol: %lf \t Local-Acc: %lf \t Vol-Acc: %lf \t Avg-Cluster: %.1lf\n", 
                    step, 
                    box[0] * box[1] * box[2], 
                    local_acc_rate, 
            ...
            // Reset counters for the next output block
            move_accepted = 0;
            vol_accepted = 0;
            local_sweeps = 0;
            cluster_sweeps = 0;
    ...
    double mean_sum = 0.0; // Initialized this to 0.0 to prevent undefined behavior
    double mean_volume;
