...
int geometric_cluster_move(void) {
    // 1. Pick a random pivot point
    double pivot[NDIM];
    for (int i = 0; i < NDIM; i++) {
        pivot[i] = get_random_double(0.0, box[i]);
    }

    // 2. Pick a random seed particle
    int seed_index = (int)get_random_double(0, n_particles);
    if (seed_index == n_particles) seed_index = n_particles - 1; // Safety catch

    // Arrays to manage the cluster
    int in_cluster[N] = {0}; // 0 = unmoved, 1 = in cluster
    int stack[N];            // Queue of particles to process
    int stack_size = 0;

    // Add seed to cluster and stack
    in_cluster[seed_index] = 1;
    stack[stack_size++] = seed_index;

    // Temporary array to hold the new positions of cluster particles
    double new_r[N][NDIM];

    // 3. Process the cluster cascade
    while (stack_size > 0) {
        // Pop the last particle off the stack
        int current_particle = stack[--stack_size];

        // Calculate its reflected position: r_new = 2*pivot - r_old
        for (int d = 0; d < NDIM; d++) {
            double dist_to_pivot = pivot[d] - r[current_particle][d];
            
            // Apply nearest image convention to the distance to the pivot
            if (dist_to_pivot > 0.5 * box[d]) dist_to_pivot -= box[d];
            if (dist_to_pivot <= -0.5 * box[d]) dist_to_pivot += box[d];

            // Calculate new coordinate and apply strict PBC (0 to box length)
            new_r[current_particle][d] = apply_pbc(pivot[d] + dist_to_pivot, box[d]);
        }

        // 4. Check for overlaps at the NEW position
        double rx = new_r[current_particle][0];
        double ry = new_r[current_particle][1];
        double rz = (NDIM == 3) ? new_r[current_particle][2] : 0.0;

        // We use brute force for the cluster overlap check because the cell list
        // represents the OLD state, and modifying it mid-cluster is unsafe.
        // Since we only loop over particles NOT in the cluster, it's fast enough.
        for (int m = 0; m < n_particles; m++) {
            if (in_cluster[m]) continue; // Ignore particles already in the cluster

            double dx = rx - r[m][0];
            double dy = ry - r[m][1];
            double dz = (NDIM == 3) ? rz - r[m][2] : 0.0;
            ...
            if (dy <= -0.5 * box[1]) dy += box[1];
            if (dx * dx + dy * dy >= diameter_squared) continue;
            ...
            if (dx * dx + dy * dy + dz * dz < diameter_squared) {
                // Collision! Add particle m to the cluster
                in_cluster[m] = 1;
                stack[stack_size++] = m;
            }
        }
    ...
    // 6. Rebuild the cell list to match the new particle positions
    build_cell_list();

    return cluster_count; // Returning the size of the cluster for tracking
}
...
int main(int argc, char* argv[]){
    ...
    FILE *csv_file = fopen("pVdata_GCA.csv", "w");
    if (csv_file == NULL) {
    ...
    for (int p_idx = 0; p_idx < num_pressures; p_idx++) {
        ...
        printf("#Step \t Vol \t\t Local-Acc \t Vol-Acc \t Avg-Cluster \n");

        int move_accepted = 0;
        int vol_accepted = 0;
        
        // Trackers for our hybrid statistics
        long long total_cluster_particles_moved = 0; 
        int cluster_sweeps = 0;
        int local_sweeps = 0;
        ...
        for (int step = 0; step < mc_steps; ++step) {
            ...
            if (get_random_double(0.0, 1.0) < 0.90) { 
                ...
                total_cluster_particles_moved += geometric_cluster_move();
                cluster_sweeps++;
            }
            ...
            // --- 3. DATA COLLECTION (Equilibration check) ---
            if(step > 10000 && step % output_steps == 0){
                mean_sum += (box[0] * box[1] * box[2]);
                samples++;
            }

            // --- 4. TERMINAL OUTPUT (10 times per pressure sweep) ---
            if (step > 0 && step % (mc_steps / 10) == 0) {
                double local_acc_rate = (local_sweeps > 0) ? 
                    (double)move_accepted / (n_particles * local_sweeps) : 0.0;
                
                double avg_cluster_size = (cluster_sweeps > 0) ? 
                    (double)total_cluster_particles_moved / cluster_sweeps : 0.0;

                printf("%d \t %.3lf \t %.3lf \t\t %.3lf \t\t %.1lf\n", 
                        step, 
                        box[0] * box[1] * box[2], 
                        local_acc_rate, 
                ...
                // Reset counters for the next terminal output block
                move_accepted = 0;
                vol_accepted = 0;
                local_sweeps = 0;
                cluster_sweeps = 0;
                total_cluster_particles_moved = 0;
                
                // write_data(step); // Optional: Uncomment to save coordinates
            }
        }
    ...
    fclose(csv_file);
    printf("\nData successfully written to pVdata_GCA.csv!\n");
